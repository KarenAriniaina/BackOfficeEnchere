window.addEventListener("load", function () {
    if (this.localStorage.getItem("token") == null && this.localStorage.getItem("idAdmin") == null) {
        ...
        if (id != undefined) {
            ...
            xhr.open("POST", "https://serveurenchere2-production.up.railway.app/DeleteCategorie/" + id, true);
                xhr.setRequestHeader("idAdmin", localStorage.getItem('idAdmin'));
                xhr.setRequestHeader("token", localStorage.getItem('token'));
                xhr.send(null);
        }
        else {
            this.alert("aucun categorie a supprimer");
            this.window.location.assign("GestionCategorie.html");
        }
    }
});